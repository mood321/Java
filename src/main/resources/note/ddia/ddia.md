...
如同南辕北辙一样, 你永远不能基于一个错误的方向得到一个正确的答案
...
4. 让我们再次回顾“可线性化”的思想：让一个系统看起来好像只有一个数据副本，且所有的操作都是原子的。这其实就是意味着操作是按照某种顺序执行。
...
根据这个定义，我们可以理解为：在可线性化数据存储中不存在并发操作，一定有一个时间线将所有操作都全序执行。可能存在多个请求处于等待处理的状态，但是数据存储保证了在特定的时间点执行特定的操作，所以是单个时间轴，单个数据副本，没有并发。

+ （2）可线性化强于因果一致性

可线性化一定意味着因果关系：任何可线性化的系统都将正确地保证因果关系。特别是，如果系统存在多个通信通道，可线性化确保了因果关系会自动全部保留，而不需要额外的工作。而因果一致性在多个通信通道之间是一种并发，无法确保因果关系。
...
显示跟踪所有因果关系不切实际：虽然因果关系很重要，但实际上跟踪所有的因果关系并不切实际。在许多应用程序中，客户端在写入之前会先读取大量数据，系统无法了解之后的写入究竟是依赖于全部读取内容，还是仅仅只需要其中一部分。但很明显，显示跟踪所有已读数据意味着巨大的运行开销。

使用序列号或时间戳来排序事件是更好的方法：递增的序列号或时间戳非常紧凑，并且它们可以保证全序关系。根据唯一且可比较的序列号或时间戳，对于任何操作，我们总是可以比较大小，从而确定顺序。
...
> 思路一：每个节点都独立产生自己的一组序列号。例如，如果有两个节点，则一个只生成奇数，另一个只生成偶数。问题是：每个节点可能有不同的处理速度，一个节点差生偶数而另一个产生奇数，偶数的计数器产生速度可能远远落后于奇数的计数器，反之亦然。这样就无法准确知道哪个操作在先。
> 
> 思路二：可以把墙上时间戳（物理时钟）附加到每个操作上。时间戳可能是不连续的，但是只要它们有足够高的分辨率，就可以用来区分操作。“最后写入者获胜”的冲突解决方案就是采用这种方法。问题是：物理时钟的时间戳会受到时钟偏移的影响，也可能导致与实际因果关系不一致。
> 
> 思路三：预先分配序列号的区间范围。例如，节点A负责区间1~ 1000的序列号，节点B负责区间1001~ 2000。然后每个节点独立地从区间中分配序列号，当序列号出现紧张时就分配更多的区间。问题是：对于存在因果关系的两个操作，一个操作可能被分配到1 ~1000之间的某个序列号，而后发生的操作可能被分配到另一个节点，拿到1001 ~2000之间的序列号，导致因果关系不一致。
...
>（2）数据库复制。全序关系广播正是数据库复制需要的：如果每条消息代表数据库写请求，并且每个副本都按照相同的顺序处理这些写请求，那么所有副本可以保持一致。该原则也被称为“状态机复制”
...
+ （4）节点回答准备请求，如果回答"是"，则节点需要保证在任何情况下都可以提交事务。 提交点。协调者收到所有节点回复"是"，作出明确的决定。协调者把决定写入到磁盘事务日志(为了崩溃恢复)。
...
真正实践中，分布式系统往往采用ZooKeeper等工具以一种类似外包的形式来实现共识。这远比自己开发共识算法要好的多。

ZooKeeper或etcd项目通常被称为"分布式键值存储"或"协调与配置服务"。它们对外提供的API和数据库非常相像：读取、写入对应主键的值、遍历主键。

应用开发者很少直接使用ZooKeeper，绝大多数情况是其他项目间接地依赖于ZooKeeper，如HBase，Hadoop YARN，OpenStack Nova，Kafka等。（ZooKeeper工具人。）
...
通过管道将程序连接起来的想法，是经典的Unix哲学，关于这种哲学的描述如下：

+ （1）每个程序做好一件事。如果要做新的工作，则建立一个全新的程序，而不是通过增加新“特征”使旧程序变得更加复杂。
+ （2）期待每个程序的输出成为另一个尚未确定的程序的输入。千万不要将输出与无关信息混淆在一起。避免使用严格的表格状或二进制输出格式。不要使用交互式输入。
+ （3）尽早尝试设计和构建软件，甚至是操作系统，最好在几周内完成。需要扔掉那些笨拙的部分时不要犹豫，并立即进行重建。
+ （4）优先使用工具来减轻编程任务，即使你不得不额外花费时间去构建工具，并且预期在使用完成后会将其中一些工具扔掉。

这种设计哲学：自动化、快速原型设计、增量式迭代、测试友好、将大型项目分解为可管理的模块等，与今天的敏捷开发和DevOps运动不谋而合，四十年都没有过时！不得不说，Unix实在是一个伟大的设计。
+  2.统一接口

 如果希望某个程序的输出成为另一个程序的输入，也就意味着这些程序必须使用相同的数据格式，即：需要兼容的接口。

在Unix中，这个接口就是文件（更准确地说，文件描述符），文件只是一个有序的字节序列。绝大部分（但不是全部）的Unix程序将这个字节序列视为ASCII文本，然后使用标准相同的记录分隔符以支持交互操作。

Unix使用统一接口可以表示很多不同的东西，使得差异明显的不同程序通过共享统一的接口，能够轻松连接在一起，这是一个不可思议的工作。今天能像Unix工具一样流畅地连接多个程序绝对是一个“异类”，而不是常态。（No country for oldman.）
 
+ 3 逻辑与布线分离

> Unix使用标准输入和标准输出。管道允许将一个进程的stdout附加到另一个进程的stdin。这允许用户以任何他们想要的方式连接输入和输出：程序并不知道也不关心输入来自哪里以及输出到哪里。将输入/输出的布线连接与程序逻辑分开，可以更容易地将小工具组合成更大的系统。

+ 4.透明与测试

> Unix工具之所以普遍且成功，很大的原因在于它可以非常轻松地观察事情的进展，对于测试非常有用。

## 3. MapReduce与分布式文件系统

Unix工具有数不尽的优点，但是最大局限在于它们只能在一台机器上运行。对于分布式系统，就是像hadoop这样的工具的工作场景。

### 3.1 MapReduce和Unix工具的联系与区别

+ （1）一方面，MapReduce有点像分布在数千台机器上的Unix工具。在Unix世界中，允许多个程序组合在一起的统一接口是文件和管道；在MapReduce中，该接口是分布式文件系统。我们看到数据流引擎添加了自己的管道式数据传输机制，以避免在分布式文件系统中将中间状态实体化，而作业的初始输入和最终输出通常仍然是HDFS。
+ （2）另一方面，MapReduce和Unix命令管道的主要区别在于它可以跨多台机器并行执行计算，其不必编写代码来指示如何并行化。mapper和reducer不需要知道输入来自哪里或者输出到什么地方，所以框架可以处理复杂的跨机器移动数据的情形。
...
+ （2）调用mapper函数，提取键值对
+ （3）键值对排序
+ （4）调用reducer函数，处理排序后的键值对

注意：步骤3的排序sort隐含在MapReduce中，无需用户自己编写，mapper的输出始终会经过排序，然后才传递给reducer。

MapReduce的分布式执行

在分布式计算中，可以使用标准的Unix工具作为mapper和reducer，但更为常见的是与用传统的编程语言实现的函数，比如hadoop MapReduce中，mapper和reducer都是实现特定接口的java类。

MapReduce的并行化基于分区实现：

+ （1）对于输入，进行map计算。作业的输入通常是hdfs中的一个目录，且输入目录中的每一个文件或文件块都被视为一个单独的分区，可以由一个单独的map任务来处理。
一个输入文件的大小通常是几百兆，只要有足够的空闲内存和CPU资源，MapReduce调度器会尝试在输入文件副本的某台机器上运行mapper任务，这个原理被称为“就近计算原理”：它避免将输入文件通过网络进行复制，减少了网络负载，提高了访问局部性。
+ （2）reduce任务中的计算也被分割成块。map任务（分区）的数量由输入文件块的数量决定，而reduce任务（分区）的数量则是由作业的作者来配置的。为了确保具有相同关键字的所有键值对都在相同的reduce任务中处理，框架使用关键字的哈希值来确定哪个reduce任务接收特定的键值对。
reducer与每一个mapper相连接，并按照其分区从mapper中下载排序后的键值对文件。按照reduce分区，排序和将数据分区从mapper复制到reducer，这样的一个过程被称为shuffle。

MapReduce工作流

单个MapReduce作业能够解决的问题非常有限，将MapReduce作业链接到工作流中是非常普遍的，这样，作业的输出将成为下一个作业的输入。

但是注意：链接方式的MapReduce作业并不像Unix流水线。Unix命令流水线直接将一个进程的输出作为输入传递至另一个进程，只需要很小的内存缓冲区，无需写文件。而链接方式是通过目录名隐式完成的：第一个作业必须配置为将其输出写入hdfs中的指定目录，而第二个作业必须配置为读取相同的目录名作为输入。从MapReduce框架的角度看，他们仍然是两个独立的作业。链接方式更像是一系列命令，其中每个命令的输出被写入临时文件，下一个命令从临时文件中读取。

为了处理工作流之间的依赖关系，已经开发了各种hadoop工作流调度器，比如Oozie，Azkaban等，而hadoop的各种高级工具，如pig，hive，cascading等，则支持设置多个MapReduce阶段的工作流，这些不同阶段的作业流会被恰当地自动链接起来，非常方便。

### 3.3 批处理的数据关联

在许多数据集中，通常一条记录会与另一条记录存在关联。在数据库中，如果查询涉及到数据关联，一般采用join操作，对多个索引进行查找。然而，MapReduce中没有索引的概念，那么涉及到数据关联该如何操作呢？

我们的目的：使用MapReduce将所有相关记录集中到一起，从而有效地处理它们。

我们的方法：分组，join

分组

“将相关数据放在一起”的一个常见用法是通过某个关键字，对记录进行分组。所有具有相同关键字的记录形成一个组，然后在每个组内执行某种聚合操作。

使用MapReduce实现这种分组操作的最简单方法是设置mapper，使其生成的键值对使用所需的分组关键字。然后，分区和排序过程将相同的reducer中所有具有相同关键字的记录集合在一起。

join

“将相关数据放在一起”的另一个最经典的用法是join。

join可分为reduce端的排序-合并join，以及map端的广播哈希join、分区哈希join、map端合并join。

reduce端join的优点是不需要对输入数据做任何假设：无论其属性与结构如何，mapper都可以将数据处理好以准备join。缺点是，对于这些排序，复制到reducer以及合并到reducer可能是非常昂贵的操作，这取决于可用的内存缓冲区，数据可能需要写入磁盘若干次。

map端join的优点在于可以加快速度，这种方法使用了一个缩减版本的MapReduce作业，其中没有reducer，也没有排序，每个mapper只需读取输入文件块，然后将输出文件写入文件系统即可。缺点在于：条件苛刻，需要对输入数据进行某些假设。

排序-合并join
>每个将要join的输入都会由一个join关键字的mapper处理，在map端经过分区、排序，然后reducer将来自join两个输入的已排序记录列表合并，结果是具有相同关键字的所有记录都最终会进入对reducer的同一个调用。然后由reducer执行真正的join操作，输出join记录。

存在的问题：数据倾斜。如果与单个关键字相关的数据量非常大，而reducer分区大小是有限的，非常可能破坏“将所有相同关键字的记录放在一起”的模式。从而形成热键。

如何处理热键与数据倾斜？

>（1）算法补偿。例如pig和crunch中，都是将与热键相关的记录分散到多个reducer上。缺点是破坏了“就近计算”原则，需要借助网络通信复制副本，性能变差。
>（2）其它join方法。比如hive，对热键使用map端join。

广播哈希join

> 两个join输入中的其中一个数据集很小，所以不需要分区，可以完全加载到哈希表中。因此，可以为大数据集的每个分区启动一个mapper，将小数据集的哈希表加载到每个mapper中，然后一次扫描大数据集的一条记录，对每条记录进行哈希表查询。
> 
> “广播”是指：大数据集每个分区的mapper都将小数聚集全部加载至内存（即小数据集实际被“广播”给大数据集）。
> 
> “哈希”意味着使用哈希表。

分区哈希join

>如果两个join的输入以相同的方式分区（使用相同的分区数量，相同的关键字，相同的哈希函数），则哈希表方法可以独立用于每个分区。

map端合并join

>如果两个join的输入不仅以相同的方式进行分区，而且还基于相同的关键字进行了排序，则可以应用map端合并join。这时，输入是否可以载入内存并不重要，因为mapper可以执行通常由reduce端执行的合并操作：按关键字升序增量读取两个输入文件，并且匹配具有相同关键字的记录。

<b>启示：不同join方式的分区和排序方法不同</b>

当下游作业使用MapReduce join输出时，map端或reduce端join的不同选择会影响到输出结构：reduce端join的输出按join关键字进行分区和排序；而map端join的输出按照与大数据集相同的方式进行分区和排序。

因此，在优化join策略时，了解分布式文件系统中数据集的物理布局非常重要。

3.4 批处理工作流的输出

批处理既不是事务处理，也不是数据分析，它的输出通常是其他类型的数据结构：

> （1）生成搜索索引：Google最初使用MapReduce的目的就是为其搜索引擎建立索引。如果需要对一组固定文档进行全文检索，则批处理是构建索引的有效方法：mapper根据需要对文档集进行分区，每个reducer构建其分区索引，并将索引文件写入分布式文件系统。
> （2）输出键值存储：目前的批处理作业最常用于构建数据库文件。批处理作业创建一个全新的数据库，并将其作为文件写入分布式文件系统中的作业输出目录。

批处理的输出遵循Unix相同的哲学：一个程序读取输入并写回输出。在这个过程中，输入保持不变，任何以前的输出都被最新的输出完全替换，并且没有其他副作用。这意味着可以随心所欲地重新运行一个命令，进行调试和测试，而不用担心扰乱系统。

得益于这样的框架，在批处理作业中的代码无需考虑容错机制的实现：框架可以保证作业的最终输出与没有发生错误的情况相同（可能不得不进行重试）。
...
在批处理的世界中，作业的输入和输出是文件。在流处理的世界中，作业的输入和输出是事件，这个事件可能是用户活动、传感器以及数据库的写操作等。它们本质上是一样的。事件可以被编码为文本字符串或json，或者某种二进制形式。通过这种编码方式，可以保存事件，还可以通过网络将事件发送到另一个节点以进行处理。

在批处理中，文件被写入一次，然后可能被多个作业读取。类似地，在流处理中，事件由生产者（发布者）生成一次，然后可能由多个消费者（订阅者）处理。

在批处理的文件系统中，文件名标识一组相关记录；在流系统中，相关的事件通常被组合成主题或流。
...
长期以来，流处理一直被用于监控目的，即希望在发生某些特定事件时收到警报。

+ 复杂事件处理（Complex Event Processing，CEP）。与正则表达式支持在字符串中搜索特定字符模式的方法类似，CEP允许制定规则，从而可以在流中搜索特定模式的事件。
> CEP 系统 https://www.51cto.com/article/680425.html
+ 传统的搜索引擎首先索引文档，然后在索引上运行查询。CEP则是反过来，查询条件先保存下来，然后文档流过查询条件。CEP系统通常使用像SQL这样的高级声明式查询语言或图形化用户界面，来描述应该检测到的事件模式。
+ 流分析。通过在固定时间间隔内进行聚合运算，获得一系列的统计指标，用于数据分析。常见的storm，streaming，flink等，都支持这种聚合分析。
+ 维护物化视图。对某个数据集导出一个特定的视图以便高效查询，并在底层数据更改时自动更新该导出视图。比如，将所有数据从始至终都视为变更日志，从而构建全新的视图。比如，kafka的日志压缩功能。原则上，任何流处理器都可以维护物化视图，但这种永久维护事件的需求和面向分析的流处理框架目的背道而驰，所以，一般不用。
...
+ （2）流和表join：一个输入流由活动事件组成，另一个是数据库变更日志。对于活动事件，join操作用来查询数据库并输出一个包含更多信息的事件。 由于join操作不能使用无限滑动窗口，在使用join进行实时数据分析的时候，很难将流中的所有数据进行join。另一方面，即使可以将两个流中的所有数据进行join操作，但当数据体量巨大的时候，其实时性也会受到影响。为了在不影响实时性的情况下还可以进行join操作，可以使用流与表之间的join操作进行数据分析。
