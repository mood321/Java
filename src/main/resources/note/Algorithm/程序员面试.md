...
<p>11 两个链表相交的问题
<p> 单链表可能有环，也可能无环。给定两个单链表的头结点head1和head2，折两个链表可能相交，也可能不想交。
<p>请实现一个函数，如果两个链表相交，请返回相交的第一个节点；如果不想交那么返回null。时间复杂度o(M+N)空间复杂度o(1)
<p> 思路1: 
<p>1.单链表是否有环
<p> 先知道两个链表是否有环  如果一个有一个没有  注定不会相交  那就只有下面两种情况
<p>2.两个无环单链表第一个相交的节点
<p> 这种尾节点一定相同  同从长度较长的链表 下标是两链表差开始比较
<p>3.两个有环单链表第一个相交的节点  这有两种情况
<p>3.1  两链表在入环前相交   即入环节点一致 ,可复用2的算法
<p>3.2 两链表在入环后相交 让其中一个入环节点绕环一周 能遇到另一个即为相交  没有遇到不相交
<p><a href="https://github.com/mood321/JavaDemo/blob/master/src/main/java/algorithm/Interview/practice02/P_2_9_addList.java"> code</a>
